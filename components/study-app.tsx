...
const groups: GroupMeta[] = [
  ...
  { key: 'exam', label: 'Exam Guide · Tổng quan', shortLabel: 'EG', order: 1 },
  {
    key: 'domain1',
    label: 'Domain 1 · Secure (30%)',
    shortLabel: 'D1',
    order: 2,
  },
  {
    key: 'domain2',
    label: 'Domain 2 · Resilient (26%)',
    shortLabel: 'D2',
    order: 3,
  },
  {
    key: 'domain3',
    label: 'Domain 3 · High-Performing (24%)',
    shortLabel: 'D3',
    order: 4,
  },
  {
    key: 'domain4',
    label: 'Domain 4 · Cost-Optimized (20%)',
    shortLabel: 'D4',
    order: 5,
  },
  {
    key: 'security',
    label: 'Thư viện · Ngày 1 Security',
    shortLabel: '01',
    order: 6,
  },
  {
    key: 'resilience',
    label: 'Thư viện · Ngày 2 Resilience',
    shortLabel: '02',
    order: 7,
  },
  {
    key: 'storage',
    label: 'Thư viện · Ngày 3 Storage & DB',
    shortLabel: '03',
    order: 8,
  },
  {
    key: 'networking',
    label: 'Thư viện · Ngày 4 Networking',
    shortLabel: '04',
    order: 9,
  },
  {
    key: 'compute',
    label: 'Thư viện · Ngày 5 Compute',
    shortLabel: '05',
    order: 10,
  },
  {
    key: 'operations',
    label: 'Thư viện · Ngày 6 Cost & Ops',
    shortLabel: '06',
    order: 11,
  },
  {
    key: 'review',
    label: 'Thư viện · Ngày 7 Mock',
    shortLabel: '07',
    order: 12,
  },
  { key: 'quick', label: 'Tra cứu tổng hợp', shortLabel: '★', order: 13 },
];
...
function groupForPath(path: string) {
  ...
  if (path === '08-EXAM-GUIDE-DOMAINS/README.md') return 'exam';
  if (path.startsWith('08-EXAM-GUIDE-DOMAINS/DOMAIN-1-')) return 'domain1';
  if (path.startsWith('08-EXAM-GUIDE-DOMAINS/DOMAIN-2-')) return 'domain2';
  if (path.startsWith('08-EXAM-GUIDE-DOMAINS/DOMAIN-3-')) return 'domain3';
  if (path.startsWith('08-EXAM-GUIDE-DOMAINS/DOMAIN-4-')) return 'domain4';
...
function titleFromMarkdown(content: string, path: string) {
  ...
  return (
    path.split('/').at(-1)?.replace(/\.md$/, '').replace(/-/g, ' ') ?? path
  );
}
...
function descriptionFromMarkdown(content: string) {
  ...
  const description = stripMarkdown(
    line ?? 'Tóm tắt kiến thức và các tình huống thường gặp trong đề thi.',
  );
...
export default function StudyApp() {
  const defaultArticle =
    articles.find(
      (article) => article.path === '08-EXAM-GUIDE-DOMAINS/README.md',
    ) ??
    articles.find((article) => article.path === 'README.md') ??
    articles[0];
  ...
  const activeArticle =
    articles.find((article) => article.id === activeId) ?? defaultArticle;
  ...
  const completionPercent = Math.round(
    (completed.size / articles.length) * 100,
  );
  ...
  const markdownComponents: Components = {
    ...
    a: ({ href = '', children }) => {
      ...
      const targetArticle = targetPath
        ? articles.find(
            (article) =>
              article.path.toLowerCase() === targetPath.toLowerCase(),
          )
        : null;
      if (targetArticle) {
        return (
      ...
      <header className="sticky top-0 z-50 border-b bg-background/92 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1600px] items-center gap-4 px-4 lg:px-6">
          ...
          <button
            ...
            onClick={() => openArticle(defaultArticle.id)}
          >
          ...
          <div className="relative ml-auto hidden w-full max-w-md md:block">
            ...
            <Input
              ...
              placeholder={`Tìm trong ${articles.length} bài học...`}
          ...
          <div className="ml-auto flex items-center gap-2 md:ml-0">
            <Badge variant="outline" className="hidden h-7 px-3 xl:inline-flex">
              <ListChecks /> 14 task Exam Guide
            </Badge>
            <Badge
              variant="secondary"
              className="hidden h-7 px-3 sm:inline-flex"
            >
      ...
      <div className="mx-auto grid max-w-[1600px] lg:grid-cols-[310px_minmax(0,1fr)] xl:grid-cols-[310px_minmax(0,1fr)_250px]">
        ...
        <main className="min-w-0 px-5 py-8 sm:px-8 lg:px-12 lg:py-12">
          <article className="mx-auto max-w-4xl">
            <div className="mb-9 border-b pb-8">
              <div className="mb-4 flex flex-wrap items-center gap-2">
                <Badge className="bg-primary/10 text-primary">
                  {activeArticle.dayLabel}
                </Badge>
                {isBeginnerArticle && (
                  <Badge variant="secondary">Giải thích cho người mới</Badge>
                )}
                <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
                  <Clock3 className="size-3.5" /> {activeArticle.minutes} phút
                  đọc
                </span>
                <span className="text-xs text-muted-foreground">
                  Bài {activeIndex + 1}/{articles.length}
                </span>
              </div>
              <h1 className="text-balance text-3xl font-extrabold tracking-[-0.035em] sm:text-4xl">
                {activeArticle.title}
              </h1>
              ...
              >
                ...
                {completed.has(activeArticle.id)
                  ? 'Đã học xong'
                  : 'Đánh dấu đã học'}
              </Button>
            </div>
            ...
            <nav
              className="mt-14 grid gap-3 border-t pt-8 sm:grid-cols-2"
            ...
            >
              {activeIndex > 0 ? (
                <Button
                  ...
                  onClick={() => openArticle(articles[activeIndex - 1].id)}
                >
                  <ArrowLeft />
              ...
              {activeIndex < articles.length - 1 && (
                <Button
                  variant="outline"
                  className="h-auto justify-end px-4 py-3"
                  onClick={() => openArticle(articles[activeIndex + 1].id)}
                >
        ...
        <aside className="sticky top-16 hidden h-[calc(100vh-4rem)] overflow-y-auto border-l px-6 py-10 xl:block">
          ...
          <nav className="space-y-1" aria-label="Mục lục trong bài">
            {headings.slice(0, 18).map((heading, index) => (
              <a
                ...
                href={`#${heading.id}`}
                className={`block border-l-2 border-transparent py-1.5 text-sm leading-5 text-muted-foreground transition hover:border-primary hover:text-foreground ${heading.level === 3 ? 'pl-5' : 'pl-3'}`}
              >
                {heading.label}
              </a>
            ))}
          ...
          <div className="mt-8 rounded-2xl border bg-card p-4">
            ...
            <p className="mt-2 text-xs leading-5 text-muted-foreground">
              Sau mỗi bài, tự nói lại decision tree mà không nhìn tài liệu.
            </p>
          </div>
          <a
            href="https://github.com/dzung-luong-ck/saa-c03-field-guide"
            target="_blank"
            rel="noreferrer"
            className="mt-4 flex items-center gap-2 px-1 text-xs text-muted-foreground hover:text-foreground"
          >
      ...
      <div className="mb-5 rounded-2xl border border-primary/15 bg-primary/[0.06] p-4">
        <div className="flex items-center justify-between gap-2 text-sm font-bold text-primary">
          <span className="flex items-center gap-2">
            <BookOpen className="size-4" /> Tiến độ 7 ngày
          </span>
          <span>{completionPercent}%</span>
        </div>
        <Progress value={completionPercent} className="mt-3" />
        <p className="mt-2 text-xs leading-5 text-muted-foreground">
          {completed.size} trên {totalArticleCount} bài đã hoàn thành.
        </p>
      </div>

      {query && (
        <p className="mb-4 px-2 text-xs text-muted-foreground">
          {visibleArticles.length} kết quả phù hợp
        </p>
      )}

      <div className="space-y-6 pb-8">
        {groups.map((group) => {
          const groupArticles = visibleArticles.filter(
            (article) => article.group === group.key,
          );
          if (groupArticles.length === 0) return null;
          return (
            <section key={group.key}>
              <p className="mb-2 px-2 text-[11px] font-bold uppercase tracking-[0.13em] text-muted-foreground">
                {group.label}
              </p>
              <div className="space-y-1">
                {groupArticles.map((article) => (
                  <button
                    ...
                    className={`group flex w-full items-start gap-3 rounded-xl px-3 py-2.5 text-left transition ${activeId === article.id ? 'bg-primary text-primary-foreground shadow-sm' : 'hover:bg-sidebar-accent'}`}
                  >
                    <span
                      className={`mt-0.5 grid size-6 shrink-0 place-items-center rounded-md text-[10px] font-extrabold ${activeId === article.id ? 'bg-white/15' : completed.has(article.id) ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-300' : 'bg-muted text-muted-foreground'}`}
                    >
